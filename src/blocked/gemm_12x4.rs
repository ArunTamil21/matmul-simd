...
///
/// The 12×4 kernel processes more rows per iteration than 4×4, giving
/// better throughput by amortizing loop overhead and improving instruction
/// pipelining. This is the default AVX2 implementation.
///
/// # Safety
///
/// Caller must ensure:
...
fn pack_b_panel(
    ...
    for p in 0..k_block {
        for idx in 0..4 {
            b_pack[p * 4 + idx] = bt[(j_start + idx) * k_total + (k_start + p)];
        }
    }
}
...
mod tests {
    ...
    fn test_gemm_12x4_correctness() {
        ...
        unsafe {
            matmul_blocked_12x4(&a, &b, &mut c_gemm, m, n, k, None, None);
        }
